...
def train_step(model: torch.nn.Module, 
  ...
  for batch, (X, y) in enumerate(dataloader):
      ...
      classLoss = loss_fn[0](y_pred[0], y[0][0])
      bboxLoss = loss_fn[1](y_pred[1], y[0][1:])
      ...
      train_cls_acc += (y_pred_class == y[0]).sum().item()/len(y_pred[0])

  # Adjust metrics to get average loss and accuracy per batch
  train_cls_loss = train_cls_loss / len(dataloader)
  train_bbox_loss = train_bbox_loss / len(dataloader) 
  train_loss = train_loss / len(dataloader)
  train_cls_acc = train_cls_acc / len(dataloader)
  return train_loss, train_cls_acc, train_cls_loss, train_bbox_loss
...
def test_step(model: torch.nn.Module, 
  ...
  with torch.inference_mode():
      ...
      for batch, (X, y) in enumerate(dataloader):
          ...
          # 2. Calculate and accumulate loss
          classLoss = loss_fn[0](test_pred_logits[0], y[0])
          bboxLoss = loss_fn[1](test_pred_logits[1], y[1])
          total_loss = W1 * classLoss + W2 * bboxLoss
          ...
          y_pred_class = torch.argmax(torch.softmax(test_pred_logits[0], dim=1), dim=1)
          test_cls_acc += (y_pred_class == y[0]).sum().item()/len(test_pred_logits[0])
  ...
  return test_loss, test_cls_acc, test_cls_loss, test_bbox_loss
...
def train(model: torch.nn.Module, 
  ...
  for epoch in tqdm(range(epochs)):
      train_loss, train_acc, train_cls_loss, train_bbox_loss = train_step(model=model,
                                          dataloader=train_dataloader,
                                          loss_fn=loss_fn,
                                          optimizer=optimizer,
                                          device=device)
      test_loss, test_acc, test_cls_loss, test_bbox_loss = test_step(model=model,
