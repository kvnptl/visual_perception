...
def train_step(model: torch.nn.Module, 
  ...
  for batch, (X, y_cls, y_bbox) in enumerate(dataloader):
      ...
      train_cls_acc += (y_pred_class == y_cls).sum().item()/len(y_pred[0])
...
def test_step(model: torch.nn.Module, 
  ...
  with torch.inference_mode():
      ...
      for batch, (X, y_cls, y_bbox) in enumerate(dataloader):
          ...
          # 2. Calculate and accumulate loss
          classLoss = loss_fn[0](test_pred_logits[0], y_cls)
          bboxLoss = loss_fn[1](test_pred_logits[1], y_bbox)
          total_loss = W1 * classLoss + W2 * bboxLoss
          ...
          test_cls_acc += (y_pred_class == y_cls).sum().item()/len(test_pred_logits[0])
